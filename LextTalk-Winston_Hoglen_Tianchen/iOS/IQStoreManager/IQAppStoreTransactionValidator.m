//
//  IQAppStoreTransactionValidator.m
//  deej
//
...
@interface IQAppStoreTransactionValidator()
@property (nonatomic, strong) NSString              *sharedSecret;
@property (nonatomic, strong) NSMutableDictionary   *completionHandlers;
@property (nonatomic, strong) NSMutableDictionary   *transactionsReceiptStorageDictionary;
@end

@implementation IQAppStoreTransactionValidator
...
- (id)init {
    ...
    if(self) {
        ...
        self.transactionsReceiptStorageDictionary = [NSMutableDictionary dictionary];
    }
    return self;
}

#pragma mark -
#pragma mark IQStoreTransactionValidator methods

- (void)validatePurchase:(SKPaymentTransaction *)transaction
       completionHandler:(IQStoreTransactionValidatorCompletionHandler)block
{
    BOOL isOk = [self isTransactionAndItsReceiptValid:transaction];
    
    if(!isOk) {
        if(block) {
            block(NO, [NSError errorWithDomain:@"com.inqbarna" code:0 userInfo:@{NSLocalizedDescriptionKey:NSLocalizedString(@"Invalid transaction", nil)}]);
        }
        return;
    }
    ...
    NSString *jsonObjectString = [[NSData dataWithContentsOfURL:[[NSBundle mainBundle] appStoreReceiptURL]] base64EncodedString];
    
    // Create the POST request payload.
    NSString *payload = [NSString stringWithFormat:@"{\"receipt-data\" : \"%@\", \"password\" : \"%@\"}",
                         jsonObjectString, self.sharedSecret];
    
    NSData *payloadData = [payload dataUsingEncoding:NSUTF8StringEncoding];
    
#ifdef IS_DISTRIBUTION
    NSString *serverURL = ITMS_PROD_VERIFY_RECEIPT_URL;
#else
    NSString *serverURL = ITMS_SANDBOX_VERIFY_RECEIPT_URL;
#warning DO NOT UPLOAD THE APP, this is validating product against sandbox!!
#endif
    
    NSLog(@"Validating against %@", serverURL);
    ...
    NSValue *key = [NSValue valueWithNonretainedObject:conn];
    [self.completionHandlers setObject:[block copy] forKey:key];
    [conn start];
}

#pragma mark -
#pragma mark IQAppStoreTransactionValidator methods
...
#pragma mark -
#pragma mark Local transaction checking
...
- (BOOL)isTransactionAndItsReceiptValid:(SKPaymentTransaction *)transaction {
    if (!(transaction && [NSData dataWithContentsOfURL:[[NSBundle mainBundle] appStoreReceiptURL]] && [[NSData dataWithContentsOfURL:[[NSBundle mainBundle] appStoreReceiptURL]] length] > 0)) {
    ...
    NSData *encodedData = [NSData dataFromBase64String:transactionPurchaseInfo];
    NSString *decodedPurchaseInfo   = [[NSString alloc] initWithData:encodedData encoding:NSUTF8StringEncoding];
    ...
    [self.transactionsReceiptStorageDictionary setObject:purchaseInfoDict forKey:transactionId];
...
- (BOOL)isTransactionIdUnique:(NSString *)transactionId {
    ...
    NSMutableDictionary *dict = [defaults objectForKey:transactionDictionary];
    if(dict == nil) {
    ...
    if (![dict objectForKey:transactionId]) {
        return YES;
    }
...
- (void)saveTransactionId:(NSString *)transactionId {
    ...
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    NSString *transactionDictionary = KNOWN_TRANSACTIONS_KEY;
    NSMutableDictionary *dictionary = [NSMutableDictionary dictionaryWithDictionary:[defaults objectForKey:transactionDictionary]];
...
#pragma mark -
#pragma mark Remote transaction checking methods
...
- (BOOL)doesTransactionInfoMatchReceipt:(NSString *)receiptString {
    ...
    NSDictionary *purchaseInfoFromTransaction = [self.transactionsReceiptStorageDictionary objectForKey:transactionIdFromVerifiedReceipt];
    ...
    if ([[UIDevice currentDevice] respondsToSelector:NSSelectorFromString(@"identifierForVendor")]) {                   // iOS 6?
        ...
        // Pre iOS 6, no hacemos verificación
        // TODO sergi he cambiado uniqueidentifier por uidevice+identifieradition del iqkit... algún poblema???
        // Pozí, que no funciona, pero na, poca cosa ;-)
//        NSString *localIdentifier           = [[UIDevice currentDevice] uniqueDeviceIdentifier];
...
//        if (![purchaseInfoUniqueId isEqualToString:localIdentifier]) {
//            // Comment this line out to test in the Simulator.
//            failCount++;
//        }
    }
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    ...
    IQStoreTransactionValidatorCompletionHandler block = [self.completionHandlers objectForKey:key];
    if(block) {
        block(NO, error);
    }
